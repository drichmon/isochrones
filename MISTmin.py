...
outpath = '/home/richmond/Isochrones/extrastuff/' #desired output for FITS files
...
path = '/home/richmond/Isochrones/FeH_0.00/'

__file__ = '/home/richmond/Isochrones/FeH_0.00/%s_%s.dat' % (survey, Age)

filename = open(__file__, 'r')

LogGlist = np.loadtxt(__file__, usecols = (3,))

minIndex = scipy.signal.argrelmin(LogGlist, order=1000)[0][0] + 1

command = 'head -n{0} {1} > {2}/{3}_{4}_cut.dat'.format(minIndex, __file__, path, survey, Age)
...
def Age_dict(columns):
    ...
    old = 0
    for ind in separation:
        age_dict[data[ind, 1]] = data[old:ind+1]
        old = ind + 1

def group_Ages(listOfAges, MIST_file, endIndex, x):
    i = 0
    j = 0 
    k = 1
    lines = []
            ...
            try:
                if i >= (x-5):
                    if i == endIndex[k]:
                        ...
                        i += 1
                        lines.append(line)
                        dictionary[listOfAges[j]] = lines[4:len(lines)-1]
                        j += 1
                        lines = []
...
def fitsTable(logAge, metallicitylist, initial_mass, Teff, log_g, _2Mass_J, _2Mass_H, _2Mass_Ks, outpath):
    ...
    tbhdu.writeto(outpath + 'MIST.fits', overwrite=True) #fits outfile



inputAge = '9.0000000000000000' #raw_input('')
metallicitylist = []
...
for file1 in sorted(os.listdir(inpath)): #this loop can be commented out to save time if temp files are already created
    if '.cmd' in file1:
        if 'plus' in file1:
            MIST_file = open(inpath +file1, 'r')
            num_skip_rows = get_skiprows(inputAge, MIST_file)
            x = num_skip_rows 
            #print type(x)
            ...
            #logAge = np.loadtxt(inpath +file1, skiprows = x, usecols = (1,))
            MIST_file = open(inpath +file1, 'r')
            metallicity = find_Metallicity(MIST_file)
            metallicitylist.append(metallicity)
            MIST_file = open(inpath +file1, 'r')
            endIndex = endIndexx(x, MIST_file)
            #print type(endIndex)
            listOfAges = sorted(list(OrderedDict.fromkeys(logAge)))
            #print type(listOfAges)
            MIST_file = open(inpath +file1, 'r')
            groupAges = group_Ages(listOfAges, MIST_file, endIndex, x)
            #print type(groupAges)
            minIndex = findMin(groupAges)
            #print minIndex
            cut(minIndex, groupAges)
            ...
            x = num_skip_rows 
            #print type(x)
            log_g = np.loadtxt(inpath +file1, skiprows = x, usecols = (4,))
            logAge = np.loadtxt(inpath +file1, skiprows = x, usecols = (1,))
            MIST_file = open(inpath +file1, 'r')
            metallicity = find_Metallicity(MIST_file)
            metallicitylist.append(metallicity)
...
for file in sorted(os.listdir(newinpath)):
    if '.dat' in file: # makes sure it only iterates over isochrone files
        #if 'plus' in file:
        MIST_file = open(newinpath +file, 'r')
        num_skip_rows = get_skiprows(inputAge, MIST_file)
        x = num_skip_rows -1
        columns = np.loadtxt(newinpath + file, skiprows = x)
